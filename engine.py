...
def main() -> None:
    ...
    if ws_disabled:
        if args.ws_disabled:
            LOGGER.warning("WebSocket disabled via CLI flag (--ws-disabled), using REST fallback")
        elif os.getenv("WS_DISABLED") == "1":
            LOGGER.warning("WebSocket disabled via environment variable (WS_DISABLED=1), using REST fallback")
        else:
